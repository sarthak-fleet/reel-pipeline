...
impl MarketingClient for SaaSMakerClient {
    fn list_marketing_posts(&self, filters: &ListFilters) -> Result<Vec<MarketingPost>> {
        ...
        if response.status() != 200 {
            ...
            return Err(anyhow!("SaaS Maker marketing list failed {}: {body}", response.status()));
        }
    ...
    fn update_marketing_post(&self, id: &str, patch: &Value) -> Result<UpdateResult> {
        ...
        if response.status() != 200 {
            let body = response.body_mut().read_to_string().unwrap_or_default();
            return Err(anyhow!("SaaS Maker marketing sync failed {}: {body}", response.status()));
        }
        Ok(UpdateResult {
            skipped: false,
            reason: None,
...
pub fn render_patch_for_marketing_post(render: &RenderPatchInput) -> Value {
    let video_url = first_video_url(&render.videos, &render.combined_videos, render.video_url.as_deref());
    let notes = format!(
